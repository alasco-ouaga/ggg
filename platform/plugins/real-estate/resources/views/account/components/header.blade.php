    ...
    <!-- Right Side Of Navbar -->
      <ul class="navbar-nav ml-auto my-2">
        ...
        @else
          ...
          @if (RealEstateHelper::isEnabledCreditsSystem() and auth('account')->user()->status)
              <li>
                  <a class="no-underline mr2 black-50 hover-black-70 pv1 ph2 db mr2 account-current-credit" style="text-decoration: none; line-height: 32px;" href="{{ route('public.account.packages') }}" title="{{ trans('plugins/real-estate::account.credits') }}">
                      <i class="far fa-credit-card mr1"></i>{{ trans('plugins/real-estate::account.buy_credits') }} <span class="badge badge-info">{{ number_format(auth('account')->user()->credits) }} {{ trans('plugins/real-estate::account.credits') }}</span>
                  </a>
              </li>
          @endif
          ...
          @if (auth('account')->user()->canPost() and auth('account')->user()->status)
              <li>
